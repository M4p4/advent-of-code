...
for (round = 0; round < ROUNDS; round++) {
  for (let i = 0; i < monkeys.length; i++) {
    ...
    for (let item of items) {
      ...
      const filter =
        worryLevel %
        monkeys.map(({ test }) => test).reduce((sum, v) => sum * v);

      if (filter % monkeys[i].test === 0) {
        const itemIndex = monkeys[i].items.indexOf(item);
        monkeys[i].items.splice(itemIndex, 1);
        monkeys[monkeys[i].onTrue].items.push(filter);
      } else {
        const itemIndex = monkeys[i].items.indexOf(item);
        monkeys[i].items.splice(itemIndex, 1);
        monkeys[monkeys[i].onFalse].items.push(filter);
      }
    }
  }
}
